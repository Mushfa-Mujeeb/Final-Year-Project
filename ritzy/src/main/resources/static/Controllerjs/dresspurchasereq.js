...
const refreshDressForm = () => {
  ...
  isValid([dressCondition]);
  dressCondition.disabled = true;
  fillDataIntoSelect2(dressCondition, 'Select Dress Condition', dressconditions, 'name', 'name', 'New');
  dress.dress_condition_id = JSON.parse(dressCondition.value); // data binding


  dressavailabilities = getServiceAjaxRequest("/dressavailability/findAll");
  isValid([dressAvailability]);
  dressAvailability.disabled = true;
  fillDataIntoSelect2(dressAvailability, 'Select Dress Availibility', dressavailabilities, 'name', 'name', 'Under - Maintenance');
  dress.dressavailability_id = JSON.parse(dressAvailability.value);
...
const buttonDressSumbit = () => {
  ...
  console.log(dress);
  // need to check form error
  let errors = checkDressFormError();
  if (errors == "") {
    //get user confirmation
    ...
    if (userResponse) {
      ...
      if (new RegExp("^[0-9]{3}$").test(postServiceResponse)) {
        ...
        dresses = getServiceAjaxRequest("/dress/findAll");
        fillDataIntoSelect2(dressNamePR, 'Select Dress', dresses, 'dress_name', 'dress_code', postServiceResponse);
        dprequest.dress_id = JSON.parse(dressNamePR.value);
        isValid([dressNamePR]);
        DressForm.reset(); // this is an id of dress form
        refreshDressForm(); // 
        $('#modalDressAdd').offcanvas("hide");
        // $('#modalDressAdd').removeClass("show");
        // $('.offcanvas-backdrop').removeClass("show");
        // hide off-canvas here
      } else {
