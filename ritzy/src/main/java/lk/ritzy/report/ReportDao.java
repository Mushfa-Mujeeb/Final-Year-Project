package lk.ritzy.report;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Query;

import lk.ritzy.dresscatalog.Dress;
import lk.ritzy.purchaserequest.PurchaseRequest;

import java.util.List;
import java.time.LocalDate;

public interface ReportDao extends JpaRepository<PurchaseRequest, Integer>{

    
    @Query(value = "select pr from PurchaseRequest pr where pr.purchase_request_status_id.id=1")
    List<PurchaseRequest> pendingPurchaseRequestsList();


    @Query(value = "select d from Dress d where d.style_id.id=?1 and d.dressavailability_id.id=?2")
    List<Dress> AvailableStyleDresses(int style,int availability);

    //query for filter dresses by rental duration(not reserved) [fittonDate - rental end date]
    @Query(value = "select new Dress(d.id, d.dress_code, d.dress_name, d.price) from Dress d where d.dressavailability_id.id=1 and d.id not in (select rhd.dress_id.id from ReservationHasDress rhd where ?1 between rhd.fitton_date and rhd.rental_end_date or ?2 between rhd.fitton_date and rhd.rental_end_date)")
    public List<Dress> getByRentalFDED(LocalDate fd, LocalDate ed); 
    

}
